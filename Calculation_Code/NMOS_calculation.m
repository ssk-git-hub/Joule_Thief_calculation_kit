function [V_out, freq, P_in, P_out, eff] = calculate_nmos_resistive(V_oc, R_in, a, L_1, R_L, beta_n, V_th, V_D)
    ...
    %   V_oc = 1;        % 1V input
    %   R_in = 100;      % 100 Ohm source resistance
    %   a = 1;           % 1:1 turns ratio
    ...
    V_ds_star = (sqrt(b1 .* (2 .* beta_n .* b2 .* b3 + b1)) - b1) ./ ...
                (beta_n .* b1 .* b2);

    % 3. Calculate peak primary current per Eq. (S.19)
    i_peak = (beta_n .* b2 .* b3 + b1 - sqrt(b1 .* (2 .* beta_n .* b2 .* b3 + b1))) ./ ...
             (beta_n .* b2.^2);
    ...
    V_out = 0.25 .* (V_ds_star - 2 .* V_D + sqrt(gamma_3));  
    
    t_off = (4 .* i_peak .* L_1) ./ (V_ds_star + 2 .* i_peak .* R_in + ...
            2 .* V_D - 4 .* V_oc + sqrt(gamma_3));
