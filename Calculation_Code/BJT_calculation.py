...
def calculate_bjt_full(V_oc, a, L_1, R_L, R_b, I_s, beta_F, beta_R, V_T, V_D):
    """
    ...
    xi_1 = V_oc + (I_s * R_b / (1 + a)) * (1 / beta_F + 1 / beta_R) - \
           (V_T / (1 + a)) * (a + np.log((a * V_T * beta_R) / (I_s * R_b)))
           
    xi_2 = (a * beta_R) / ((1 + a) * beta_F) * np.exp(xi_1 / V_T)
    
    # Solve for V_CE* at peak current per Eq. (S.11)
    # lambertw returns complex by default, we take the real part
    W_val = np.real(lambertw(xi_2))
    V_CE = xi_1 - V_T * W_val
    
    # 2. Calculate V_BE* per Eq. (S.10)
    V_BE = np.log((a * V_T * beta_R) / (I_s * R_b)) * V_T + V_CE
    
    # 3. Calculate peak currents using Ebers-Moll model
    # i_BE per Eq. (S.5), i_peak per Eq. (S.12)
    i_BE = I_s * ((-1 + np.exp(V_BE / V_T)) / beta_F + \
           (-1 + np.exp((V_BE - V_CE) / V_T)) / beta_R)
    i_peak = I_s * (np.exp(V_BE / V_T) - np.exp((V_BE - V_CE) / V_T) - \
             (-1 + np.exp((V_BE - V_CE) / V_T)) / beta_R)
    ...
    V_out = (V_oc - V_D) + (i_peak * R_L - 2 * (V_oc - V_D)) / \
            (1 + gamma_1 + np.sqrt(gamma_2))
            
    t_off = (i_peak * L_1 * (1 + gamma_1 + np.sqrt(gamma_2))) / \
...
if __name__ == "__main__":
    ...
    print(f"BJT Model: SMMBT3904LT1G")
